...
await controller.getSafetyConfig()
assert.deepEqual(calls[11], ['safety.config.get', {}])

await controller.updateSafetyConfig({ patch: { maxChatsPerHour: 5 } })
assert.deepEqual(calls[12], ['safety.config.update', { patch: { maxChatsPerHour: 5 } }])

await controller.getAgentStatus()
assert.deepEqual(calls[13], ['agent.status', {}])

await controller.listApprovals({ includeAll: true, kind: 'AUTO_REPLY', status: 'PENDING' })
assert.deepEqual(calls[14], ['approval.list', { includeAll: true, kind: 'AUTO_REPLY', status: 'PENDING' }])

await controller.getApproval({ id: 'approval-1' })
assert.deepEqual(calls[15], ['approval.get', { id: 'approval-1' }])

await controller.approveApproval({ id: 'approval-1', reason: 'approved' })
assert.deepEqual(calls[16], ['approval.approve', { id: 'approval-1', reason: 'approved' }])

await controller.rejectApproval({ id: 'approval-1', reason: 'rejected' })
assert.deepEqual(calls[17], ['approval.reject', { id: 'approval-1', reason: 'rejected' }])

await controller.resumeSafety({ reason: 'operator verified' })
assert.deepEqual(calls[18], ['safety.resume', { reason: 'operator verified' }])
