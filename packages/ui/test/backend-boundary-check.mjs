...
assert.doesNotMatch(bootstrapSource, /installLaunchdSupervisor|installBackendUpdate|ggrd-bootstrap/, 'Electron must not install or update the separate Runtime')
...
assert.match(electronMainSource, /ensureBackendReady\(\)[\s\S]*?openSettingWindow/, 'Electron must wait for the separate Runtime backend before opening a window')
...
assert.doesNotMatch(builderSource, /ggrd-bootstrap/, 'Electron artifacts must not ship the separate Runtime')
assert.match(builderSource, /!resources\/ggrd-\*\/\*\*/, 'Electron artifacts must exclude all Runtime build directories')
